...
contract Poly {
    ...
    string public a_addr;
    string public x_addr;

    uint public rev;
    uint public dep;

    address payable worker;
    string public result_proof_root;
    string public result_addr;
    ...
    constructor(string memory _a_addr, string memory _x_addr, uint _dep, uint _t1, uint _t2) public payable {
        user = msg.sender;
        a_addr = _a_addr;
        x_addr = _x_addr;
        rev = msg.value;
    ...
    function submit_result(string memory _result_addr, string memory _result_proof_root) public payable {
        require(now <= t1, "invalid time");
        require(msg.value == dep, "invalid deposit");

        result_proof_root = _result_proof_root;
        result_addr = _result_addr;
        worker = msg.sender;
    }

    function verify(uint[] memory _path_spec) public {
        require(now <= t2, "invalid time");
        // path_spec = _path_spec
        wait_proof = true;
        emit GetPath(result_addr, _path_spec);
    }

    event GetPath(string result_addr, uint[] _path_spec);

    function feed_proof_path(bytes[] memory _vals, bytes[] memory _avals, bytes[] memory _cvals, bytes[] memory _x, int[] memory _p) public {
        // require(check_hash(path), "invalid path");
        emit Debug("before");

        node[] memory path = new node[](_vals.length);

        for (uint i = 0; i < _vals.length; i++) {
            path[i].val = BigNumber._new(_vals[i], false, false);
            path[i].aval = BigNumber._new(_avals[i], false, false);
            path[i].cval = BigNumber._new(_cvals[i], false, false);
            path[i].x = BigNumber._new(_x[i], false, false);
            path[i].p = _p[i];
        }
        emit Debug("after build path array");
        require(check_path(path), "verification failed");
    ...
    // function check_hash(node[] path) private returns (bool) {
    //     return true;
    // }
    ...
    function verify_node(node memory _node) private view returns (bool) {
        BigNumber.instance memory bn = _node.x;
        for(int i = 1; i < _node.p; i++) {
            bn = BigNumber.bn_mul(bn, _node.x);
        }
        return BigNumber.cmp(_node.val, BigNumber.prepare_add(BigNumber.bn_mul(bn, _node.aval), _node.cval), false) == 0;
    }

    function claim_tokens() public {
        if (halt) return;

        if (now > t1 && worker == address(0)) {
            user.transfer(rev);
            halt = true;
            return;
        }
